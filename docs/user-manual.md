...
Once computed for the first time, the magnification parameters are stored for all particles and used to convert all measurements. If you need to recompute the magnification parameters, you can do so by clicking on the `Update magnification` button.
...
<!-- Particles added after the magnification update will use the new parameters, while the old particles will keep the old parameters. -->
